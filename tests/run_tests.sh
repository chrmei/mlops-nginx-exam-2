...
if [ $TESTS_FAILED -eq 1 ]; then
    echo -e "${RED}Some original tests failed.${NC}"
else
    echo -e "${GREEN}All original tests passed successfully!${NC}"
fi
...
# --- Custom Test 1: Rate Limiting (Burst Protection) ---
echo "
--- Running Custom Test 1: Rate Limiting (Burst Protection) ---"
...
echo "Sleeping for 2 seconds to allow rate limiting to free up from previous tests..."
sleep 2
...
# expecting at least 10 requests to be blocked (demonstrating rate limiting)
# With rate=10r/s and burst=2, only ~12 requests should succeed when sent rapidly. but could be more due to previoous tests.
...
# --- Custom Test 2: Rate Limiting (Within Limit) ---
echo "
--- Running Custom Test 2: Rate Limiting (Within Limit) ---"
...
if [ $TESTS_FAILED -eq 1 ]; then
    echo -e "${RED}Some custom tests failed.${NC}"
    exit 1
else
    echo -e "${GREEN}All custom tests passed successfully!${NC}"
    exit 0
fi